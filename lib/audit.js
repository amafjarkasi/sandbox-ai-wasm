...
class AuditLogger {
  ...
  startExecutionAudit(executionId, params) {
    const audit = {
      ...
      astTelemetry: params.astSummary || null,
      events: [],
      securityFindings: [],
      actions: [],
      status: "started",
    ...
    if (this.executionAudits.size >= this.maxEntries) {
      const oldestKey = this.executionAudits.keys().next().value;
      this.executionAudits.delete(oldestKey);
    }
  ...
  _persistEntry(entry) {
    ...
    fsPromises.appendFile(logFile, JSON.stringify(entry) + "\n")
      .catch(e => console.error("Failed to persist audit entry:", e.message));
  }

  _persistAudit(executionId, audit) {
    const auditFile = path.join(this.logDir, `execution-${executionId}.json`);
    fsPromises.writeFile(auditFile, JSON.stringify(audit, null, 2))
      .catch(e => console.error("Failed to persist audit:", e.message));
  }
